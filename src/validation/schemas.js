...
const schemas = {
  ...
  room: Joi.object({
    ...
    campus: Joi.string().allow("").max(120),
    distanceToCampusKm: Joi.number().min(0).max(100).optional(),
    walkingTimeMinutes: Joi.number().min(0).max(300).optional(),
    cyclingTimeMinutes: Joi.number().min(0).max(300).optional(),
    transitTimeMinutes: Joi.number().min(0).max(300).optional(),
    ...
    foodType: Joi.string().valid("Veg", "Non-veg", "Both", "None").optional(),
    foodChargeMonthly: Joi.number().min(0).max(50000).optional(),
    isZeroBrokerage: Joi.boolean().optional(),
    roomType: Joi.string().valid("Single Room", "Shared Room", "PG", "Hostel", "Flat").required(),
    sharingType: Joi.string().valid("Single Room", "2-Sharing", "3-Sharing", "4-Sharing", "Private Flat").optional(),
    ...
    totalBeds: Joi.number().min(1).max(50).optional(),
    availableBeds: Joi.number().min(0).max(50).optional(),
    immediateMoveIn: Joi.boolean().optional(),
    ...
    curfewTime: Joi.string().allow("").max(60).optional(),
    noticePeriod: Joi.string().allow("").max(60).optional(),
    contactPerson: Joi.string().allow("").max(80).optional(),
    contactPhone: Joi.string().allow("").max(20).optional(),
    whatsappNumber: Joi.string().allow("").max(20).optional(),
    callingHours: Joi.string().allow("").max(80).optional(),
    images: Joi.array().items(Joi.string()).min(1).max(12).required(),
